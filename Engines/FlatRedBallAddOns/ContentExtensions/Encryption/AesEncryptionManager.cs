...
namespace FlatRedBall.ContentExtensions.Encryption
{
    public class AesEncryptionManager
    {
        ...
        public int GetPadding(byte[] decryptedBytes)
        {
            ...
            {
                var bytesToRead = decryptedBytes[decryptedBytes.Length - 1];
                bool isPadded = true;
                // we already read the last
                var bytesLeftToRead = bytesToRead - 1;

                for (int i = decryptedBytes.Length - 1; i > -1 && bytesLeftToRead > 0; i--)
                {
                    if(decryptedBytes[i] != bytesToRead)
                    {
                        isPadded = false;
                        break;
                    }
        ...
        public byte[] DecryptFromStream(Stream stream, int encryptedByteCount, out int decryptedByteCount)
        {
            // Create the streams used for decryption. 
            using (var csDecrypt = GetDecryptStream(stream))
            {
                byte[] toReturn = new byte[encryptedByteCount];
                decryptedByteCount = csDecrypt.Read(toReturn, 0, encryptedByteCount);
        ...
        public CryptoStream GetDecryptStream(Stream encryptedStream)
        {
        ...
        public byte[] DecryptFromBytes(byte[] encryptedBytes, out int decryptedByteCount)
        {
            using (MemoryStream encryptedStream = new MemoryStream(encryptedBytes))
            {
                return DecryptFromStream(encryptedStream, encryptedBytes.Length, out decryptedByteCount);
            }
        }
