...
#if SILVERLIGHT
        internal static void DrawShapes(IList<Line> lines, IList<AxisAlignedRectangle> rectangles, 
            IList<Circle> circles, IList<Polygon> polygons)
        {
            ...
            // This code is all reactive.  It keeps it local to one part of the engine, but MAY
            // suffer performance hits.  For now, keep it like that and let's see what happens.

            if (mCanvas == null)
            {
                mCanvas = sSpriteBatch.GetCanvas().Canvas;
            }

            // Vic says:  I'm not sure why the canvas' translation matrix that is set
            // previously doesn't apply to shapes.  There's some general weirdness
            // here, but I got it working so it's ok for now.
            Microsoft.Xna.Framework.Matrix matrix = camera.GetLookAtMatrix();
           
            float yRatioToMultiplyByForCameraModification = camera.DestinationRectangle.Height /
                camera.OrthogonalHeight;
            ...
            #region Create and destroy the necessary Silverlight Lines

            while (mSilverlightLines.Count < lines.Count)
            {
                System.Windows.Media.SolidColorBrush scb =
                    new System.Windows.Media.SolidColorBrush(System.Windows.Media.Colors.Green);

                System.Windows.Shapes.Line lg;


                lg = new System.Windows.Shapes.Line();
                lg.Fill = scb;
                lg.Stroke = scb;
                lg.StrokeThickness = 1;
                lg.X2 = 200;
                lg.Y2 = 200;
                lg.SetValue(Canvas.ZIndexProperty, 32000);

                Canvas canvas = mCanvas;

                canvas.Children.Add(lg);
                mSilverlightLines.Add(lg);
            }

            while (mSilverlightLines.Count > lines.Count)
            {
                int indexToRemove = mSilverlightLines.Count - 1;

                mCanvas.Children.Remove(mSilverlightLines[indexToRemove]);
                mSilverlightLines.RemoveAt(indexToRemove);
            }

            #endregion

            #region Create and destroy the necessary Silverlight Rectangles

            while (mSilverlightRectangles.Count < rectangles.Count)
            {
                ...
                System.Windows.Media.SolidColorBrush scb =
                    new System.Windows.Media.SolidColorBrush(Color.FromArgb(0, 0, 0, 0));
                rect.Fill = scb;
                ...
                rect.Stroke = scb;
                rect.StrokeThickness = 1f;
                rect.SetValue(Canvas.ZIndexProperty, 32000);

                Canvas canvas = mCanvas;

                canvas.Children.Add(rect);
                mSilverlightRectangles.Add(rect);
            }

            while (mSilverlightRectangles.Count > rectangles.Count)
            {

                int indexToRemove = mSilverlightRectangles.Count - 1;

                mCanvas.Children.Remove(mSilverlightRectangles[indexToRemove]);
                mSilverlightRectangles.RemoveAt(indexToRemove);
            }

            #endregion

            #region Create and destroy the necessary Silverlight Circles

            while (mSilverlightCircles.Count < circles.Count)
            {
                System.Windows.Media.SolidColorBrush scb =
                ...
                ellipse.SetValue(Canvas.ZIndexProperty, 32000);
                Canvas canvas = mCanvas;
                canvas.Children.Add(ellipse);
                mSilverlightCircles.Add(ellipse);
            }

            while (mSilverlightCircles.Count > circles.Count)
            {

                int indexToRemove = mSilverlightCircles.Count - 1;

                mCanvas.Children.Remove(mSilverlightCircles[indexToRemove]);
                mSilverlightCircles.RemoveAt(indexToRemove);
            }
            #endregion

            #region Create and destroy the necessary Silverlight Polygons

            while (mSilverlightPolylines.Count < polygons.Count)
            {
                ...
                polyline.SetValue(Canvas.ZIndexProperty, 32000);
                mCanvas.Children.Add(polyline);

                mSilverlightPolylines.Add(polyline);
            }

            while (mSilverlightPolylines.Count > polygons.Count)
            {
                int indexToRemove = mSilverlightPolylines.Count - 1;

                mCanvas.Children.Remove(mSilverlightPolylines[indexToRemove]);
                mSilverlightPolylines.RemoveAt(indexToRemove);
            }
            #endregion


            #region Update the line positions.  This is pretty much the every-frame drawing code

            for (int i = 0; i < lines.Count; i++)
            {
                Point3D point1 = lines[i].AbsolutePoint1;
                Point3D point2 = lines[i].AbsolutePoint2;

                mSilverlightLines[i].X1 = (float)point1.X;
                mSilverlightLines[i].Y1 = -(float)point1.Y;
                mSilverlightLines[i].X2 = (float)point2.X;
                mSilverlightLines[i].Y2 = -(float)point2.Y;

                mSilverlightLines[i].StrokeThickness = 1 / yRatioToMultiplyByForCameraModification;
                mSilverlightLines[i].SetValue(Canvas.RenderTransformProperty, transform);

                ((SolidColorBrush)mSilverlightLines[i].Fill).Color = Color.FromArgb(
                    255, lines[i].Color.R, lines[i].Color.G, lines[i].Color.B);

            }

            #endregion

            #region Update the rectangle positions.  Every-frame drawing code.

            for (int i = 0; i < rectangles.Count; i++)
            {
                System.Windows.Shapes.Rectangle silverlightRect = mSilverlightRectangles[i];
                AxisAlignedRectangle aaRect = rectangles[i];

                SolidColorBrush strokeBrush = (SolidColorBrush)silverlightRect.Stroke;
                strokeBrush.Color = aaRect.Color.ToSilverlightColor();
                
                ((SolidColorBrush)silverlightRect.Fill).Color = aaRect.FillColor.ToSilverlightColor();
                
                silverlightRect.Width = 2 * aaRect.ScaleX;
                silverlightRect.Height = 2 * aaRect.ScaleY;
                silverlightRect.StrokeThickness = 1 / yRatioToMultiplyByForCameraModification;

                float top =
                    -aaRect.Y - aaRect.ScaleY;
                float left =
                    aaRect.X - aaRect.ScaleX;

                left *= matrix.M11;
                top *= matrix.M22;


                silverlightRect.SetValue(Canvas.LeftProperty, (double)left);
                silverlightRect.SetValue(Canvas.TopProperty, (double)top);
                silverlightRect.RenderTransform = transform;

            }

            #endregion

            #region Update the circle positions.  Every-frame drawing code.
            for (int i = 0; i < circles.Count; i++)
            {
                System.Windows.Shapes.Ellipse silverlightEllipse = mSilverlightCircles[i];
                Circle circle = circles[i];

                ((SolidColorBrush)silverlightEllipse.Stroke).Color = circle.Color.ToSilverlightColor();
                ((SolidColorBrush)silverlightEllipse.Fill).Color = circle.FillColor.ToSilverlightColor();

                silverlightEllipse.Width = circle.Radius * 2;
                silverlightEllipse.Height = circle.Radius * 2;

                silverlightEllipse.StrokeThickness = 1 / yRatioToMultiplyByForCameraModification;
                silverlightEllipse.SetValue(Canvas.RenderTransformProperty, transform);
                ...
                silverlightEllipse.SetValue(Canvas.LeftProperty, (double)left);
                silverlightEllipse.SetValue(Canvas.TopProperty, (double)top);
            }

            #endregion

            #region Update the Polygon positions.  Every-frame drawing code.

            for (int i = 0; i < polygons.Count; i++)
            {
                Polygon polygon = polygons[i];

                System.Windows.Shapes.Polyline polyline = mSilverlightPolylines[i];

                ((SolidColorBrush)polyline.Stroke).Color = polygon.Color.ToSilverlightColor();

                polyline.StrokeThickness = 1 / yRatioToMultiplyByForCameraModification;
                polyline.SetValue(Canvas.RenderTransformProperty, transform);

                #region Match the points on the polyline with the points on the Polygon

                if (polyline.Points.Count != polygon.Points.Count)
                {
                ...
                // now adjust the points in case they've changed or the Polygon has rotated

                for (int pointIndex = 0; pointIndex < polygon.Points.Count; pointIndex++)
                {
                    polyline.Points[pointIndex] =
                        new System.Windows.Point(
                            polygon.Vertices[pointIndex].Position.X,
                        // Positive Y is down in Silverlight
                            -polygon.Vertices[pointIndex].Position.Y);
                }
                #endregion
...
#if SILVERLIGHT
        static List<System.Windows.Shapes.Line> mSilverlightLines = 
            new List<System.Windows.Shapes.Line>();
        static List<System.Windows.Shapes.Rectangle> mSilverlightRectangles =
            new List<System.Windows.Shapes.Rectangle>();

        static List<System.Windows.Shapes.Ellipse> mSilverlightCircles =
            new List<System.Windows.Shapes.Ellipse>();

        static List<System.Windows.Shapes.Polyline> mSilverlightPolylines =
