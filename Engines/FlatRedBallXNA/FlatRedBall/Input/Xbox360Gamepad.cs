...
public class Xbox360GamePad : IInputDevice
{
    ...
    {
        return GetButtonPushed(ignoreDirectionals, ignoreAnalogs) != null;
    }

    public Button? GetButtonPushed(bool ignoreDirectionals = false, bool ignoreAnalogs = false)
    { 
    ...
    public async Task<Button?> WaitForAnyButtonPushed(bool ignoreDirectionals = false, bool ignoreAnalogs = false,
        CancellationToken cancellationToken = default)
    {
        Button? buttonPushed = null;

        await TimeManager.DelayUntil(() =>
        {
            var buttonPushedInner = GetButtonPushed();

            if (buttonPushedInner != null)
            {
                buttonPushed = buttonPushedInner.Value;
                return true;
            }
