...
namespace FlatRedBall.Utilities
{
	...
	public class StringEnum
	{
...
#if WINDOWS_8
            foreach (FieldInfo fi in _enumType.GetRuntimeFields())
#else
		...
		public IList GetListValues()
		{
            ...
            List<DictionaryEntry> values = new List<DictionaryEntry>();
			//Look for our string value associated with fields in this enum
#if WINDOWS_8
            foreach (FieldInfo fi in _enumType.GetRuntimeFields())
#else
...
#endif
            {
				...
				StringValueAttribute[] attrs = fi.GetCustomAttributes(typeof (StringValueAttribute), false) as StringValueAttribute[];
				if (attrs.Length > 0)
					values.Add(new DictionaryEntry(
                        Convert.ChangeType(Enum.Parse(_enumType, fi.Name, false), underlyingType, System.Globalization.CultureInfo.CurrentCulture), 
                        attrs[0].Value));
...
#if SILVERLIGHT || WINDOWS_PHONE || XBOX360 || WINDOWS_8
