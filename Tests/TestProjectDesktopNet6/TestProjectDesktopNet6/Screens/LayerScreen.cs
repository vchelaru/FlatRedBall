...
public partial class LayerScreen
{
    ...
    private void TestLayeredShapes()
    {
        LayerForShapes.AxisAlignedRectangles.Count().ShouldNotBe(0);
        LayerForShapes.AxisAlignedRectangles.Contains(LayeredAARect).ShouldBe(true);

        LayerForShapes.Circles.Count().ShouldNotBe(0);
        LayerForShapes.Circles.Contains(LayeredCircle).ShouldBe(true);
