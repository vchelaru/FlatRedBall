...
3. **Testability gate — two sub-checks, in order.** (a) Can the fix be pinned with a real unit test as the code stands? (b) If not, that's expected — this codebase is missing DI/interfaces in places — so propose and land a scoped refactor task *before* touching the bug, then re-check (a). See [REFACTORING.md](../../../FRBDK/Glue/REFACTORING.md) for the incremental-refactor philosophy and the transitional-injection pattern (`Xyz.Self` defaulting field with an internal setter) used to unstick static-singleton coupling. Do not write the bug fix itself until both sub-checks pass. **Guard this concretely: before your first `Edit`/`Write` to the buggy source file, state which failing test you just watched fail. If you can't state one, stop and write/run it first** — a confident diagnosis is not a substitute for this checkpoint.
   ...
   **GlueControl-embedded/generated code has its own seam — check it before declaring manual-test-only.** A change to `Embedded/**/*.cs` or `EmbeddedCodeManager.cs` isn't automatically stuck at (b): `GoldProject.cs`'s `EmbedLiveEditCode()` drives the real `EmbeddedCodeManager.EmbedAll` against a real copied sample project, `GoldProject.GeneratedFiles()` lists the `.Generated.cs` files it produced, and `GoldProjectCompileTests.cs` already builds the result headlessly.
   **An unpinned diagnosis is itself the finding.** When the report has no stack trace and the crash could sit in any of several candidate paths, do not stop at "need a repro": write a pinning test for every candidate that has none (each guess rules a path in or out and stays as coverage), and if none goes red, add the diagnostic (crash log, output-tab trace) that would have made the site obvious so the next report arrives with the answer.
