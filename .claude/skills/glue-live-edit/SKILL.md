...
- **Edits made while no game is connected (mid-build, or during a reconnect) are caught up on connect** by `Managers\CompiledElementSweep.cs`. It compares each element's `GlueSourceHash`, compiled into the generated code as an `AssemblyMetadata` attribute, with Glue's current hash. Only element `CustomVariable`s are resent, and only when hot reload is available; it never restarts the game, since a spurious hash mismatch would restart it in a loop.
