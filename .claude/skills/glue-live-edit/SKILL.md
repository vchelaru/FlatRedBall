...
Suspecting a hard-to-repro live-edit bug rather than a socket problem? The Build tab's **Embedded diagnostics** checkbox (`BuildTabView.xaml`, `CompilerViewModel.IsEmbeddedDiagnosticsChecked`) toggles `EmbeddedDiagnosticsLogger` (game-side, `Embedded\Editing\EditingManager.cs:1635`), which appends events to `%LOCALAPPDATA%\FlatRedBall\Glue\Diagnostics\communication-{timestamp}.log` as they happen. Currently logs click attempts and selection changes only. It's per-process state on the game side: toggling the checkbox mid-session sends `SetEmbeddedDiagnosticsEnabledDto`, but a freshly-launched game never learns the checkbox was already checked, so clicks silently go unlogged with no error (`MainCompilerPlugin.cs:1246-1259`).
