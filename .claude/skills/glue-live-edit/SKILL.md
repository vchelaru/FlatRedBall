...
- **A reply's `Succeeded` is the only readiness signal; its payload is not.** Most `HandleDto` overloads
  ...
  fails an empty reply, since a typed caller asked for data.
...
- **`zoomLevel` never reflects the selected zoom.** `UpdateCameraToZoomLevel`'s local `zoomLevel` only gets assigned inside a branch gated by `makeDefaultCamera100`, hardcoded `false` — so it stays `100` no matter what zoom the user picked, while `Camera.Main.OrthogonalHeight` is correctly driven by a separate `divisor` value. Since Gum's `Renderer.Camera.Zoom`/`LayerCameraSettings.Zoom` are set from `zoomLevel`, not `divisor`, Gum content in edit mode never zooms with the user's selection — only window resizing affects it.
- **No separate zoom axis exists for world-space vs. screen-space Gum content.** `LayerCameraSettings.IsInScreenSpace` is already honored correctly at render time in `SpriteRenderer.GetZoomAndMatrix` (screen-space defaults `Zoom` to 1, world-space tracks `camera.Zoom`) — but `CameraLogic`'s per-layer loop and generated `CameraSetup.ResetGumResolutionValues` stamp the same `Zoom` onto every layer regardless, and Glue's codegen never sets `IsInScreenSpace` anywhere. HUD and in-world Gum objects share `Renderer.MainLayer` by default, so there's no way to zoom world-space Gum content independently of HUD content.
