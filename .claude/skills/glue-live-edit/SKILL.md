...
Suspecting a hang or a resource leak rather than a socket problem? The Build tab has a **Resource diagnostics** checkbox (`CompilerPlugin`, `ResourceDiagnosticsManager` + `CompilerLibrary.Diagnostics`) that samples USER/GDI object counts, handle and thread counts, and UI-thread stall time on a background thread, appending every sample to a log file under `%LOCALAPPDATA%\FlatRedBall\Glue\Diagnostics`. Reach for the file, not the tab: when this fires the UI is frozen and unreadable. USER/GDI objects cap at 10,000 per process, and exhausting them makes `PostMessage` fail with `ERROR_NOT_ENOUGH_QUOTA` (Win32 1816) — which looks like a hang but is not one.
