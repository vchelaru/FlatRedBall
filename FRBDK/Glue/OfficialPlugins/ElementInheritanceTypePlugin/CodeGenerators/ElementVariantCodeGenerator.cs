...
internal class ElementVariantCodeGenerator : ElementComponentCodeGenerator
{
    ...
    private void CreateDerivedVariantClass(IElement element, string typeOrVariant, ICodeBlock classBlock, GlueElement derivedElement)
    {
        ...
        AddGetFileAssignment(derivedElement, block);
    ...
    private void AddGetFileAssignment(GlueElement derivedElement, ICodeBlock block)
    {
        string recursiveSuffix = string.Empty;

        var baseElement = ObjectFinder.Self.GetBaseElement(derivedElement);
        while (baseElement != null)
        {
            recursiveSuffix += $" ?? {QualifiedTypeName(baseElement)}.GetFile(fileName)";
            baseElement = ObjectFinder.Self.GetBaseElement(baseElement);
        }

        block.Line($"GetFile = fileName => {QualifiedTypeName(derivedElement)}.GetFile(fileName) {recursiveSuffix},");
    }
