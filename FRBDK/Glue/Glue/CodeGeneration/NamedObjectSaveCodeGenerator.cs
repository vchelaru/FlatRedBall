    ...
    public class NamedObjectSaveCodeGenerator : ElementComponentCodeGenerator
    {
        ...
        public static void GetActivityForNamedObject(NamedObjectSave namedObjectSave, ICodeBlock codeBlock)
        {
            ...
            //if (!setByDerived)
            {
                ...
                {
                    var namedObjectAssetTypeInfo = namedObjectSave.GetAssetTypeInfo();
                    // Update 2/13/2021
                    // If it is SetByDerived, then the derived may be responsible for it,
                    // and if we call activity in the base, the object may get double activities called.
                    // This happened with TileMaps where the base GameScreen set the Map to SetByDerived, then the derived screen set
                    // the Map to be from file. The base screen was calling Activity and so was the derived on the file.
                    if (!namedObjectSave.SetByDerived && namedObjectAssetTypeInfo?.ActivityMethod != null)
                    {
                        var activityMethod = namedObjectAssetTypeInfo.ActivityMethod;
                        // Update 2/13/2021
                        // If an object comes from a file, the file
                        // will call its activity. If it doesn't come from a 
                        // file, we need to make sure that it is not SetByDerived.
                        var isHandledByFile = namedObjectSave.SourceType == SourceType.File;

                        if(isHandledByFile)
                        {
                            // it's only handled by the file IF it's a screen
                            isHandledByFile = namedObjectSave.GetContainerType() == ContainerType.Screen;
                        }
