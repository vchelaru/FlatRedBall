...
public class GluxCommands : IGluxCommands
{
        ...
        )
    {
        ...
        if (!failed)
        {
            if (isBuiltFile)
            {
                ...
                ProjectLoader.Self.SetExternallyBuiltFileIfHigherThanCurrent(directoryOfFile, false);
            }
        }
    ...
    public async void DeleteFolderClick(ITreeNode targetNode)
    {
        ...
        {
            shouldDelete = GlueCommands.Self.DialogCommands.ShowYesNoMessageBox(String.Format(
                "The {0} directory is not empty. Are you sure you want to delete it and everything in it?", absolutePath),
                "Are you sure?");
        }
    ...
    public async Task RemoveFromProjectOptionalSaveAndRegenerate(bool saveAndRegenerate = true, bool askAreYouSure = true, bool askToDeleteFiles = true)
    {
    ...
    public async Task MoveSelectedObjectUp()
    {
        int direction = -1;
        await MoveObjectInDirection(direction);
    }

    public async Task MoveSelectedObjectDown()
    {
        int direction = 1;
        await MoveObjectInDirection(direction);
    }
    ...
    private static async Task MoveObjectInDirection(int direction)
    {
        await TaskManager.Self.AddAsync(() =>
        {
            ...
            String.Format("Moving {0} up or down", GlueState.Self.CurrentCustomVariable?.ToString() ?? GlueState.Self.CurrentNamedObjectSave?.ToString()),
