...
namespace FlatRedBall.Glue.FormHelpers
{
    ...
    {
        [Obsolete("Lucky Scamper 09-23-23: remove thsi method once System.Web.Forms -> TreeView's are no longer used.")]
        ...
        public static void SortByTextConsideringDirectories(this ItemCollection treeNodeCollection, bool recursive = false)
        {
            ...
            for (int i = 0 + 1; i < lastObjectExclusive; i++)
            {
                ...
                if (TreeNodeComparer(first, second) < 0)
                {
                    ...
                    for (int whereObjectBelongs = i - 2; whereObjectBelongs > -1; whereObjectBelongs--)
                    {
        ...
        private static int TreeNodeComparer(TreeViewItem first, TreeViewItem second)
        {
