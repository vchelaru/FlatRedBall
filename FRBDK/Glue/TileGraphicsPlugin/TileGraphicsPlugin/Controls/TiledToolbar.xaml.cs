...
public class TiledToolbarViewModel : ViewModel, ISearchBarViewModel
{
    ...
    public bool IsOpenNewTiledInstanceChecked
    {
        get => Get<bool>();
        set => Set(value);
    }
    ...
    public TiledToolbarViewModel()
    {
        // default to true, because this avoids a common Tiled bug
        IsOpenNewTiledInstanceChecked = true;
    }
    ...
    internal void HandleTmxSelected(TmxReferenceViewModel vm)
    {
        var assocation = FileAssociation.GetExecFileAssociatedToExtension(".tmx");

        var startInfo = new ProcessStartInfo();
        if (IsOpenNewTiledInstanceChecked && !string.IsNullOrEmpty(assocation))
        {
            startInfo.FileName = assocation;
            startInfo.ArgumentList.Add(vm.FilePath.FullPath);
...
public partial class TiledToolbar : UserControl
{
    ...
    private void SelectTmxReference(TmxReferenceViewModel vm)
    {
        this.ViewModel.HandleTmxSelected(vm);
    }
