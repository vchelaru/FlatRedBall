...
public class TiledObjectTypeCreator
{
    ...
    private TiledObjectTypeSave CreateTiledObjectTypeFrom(EntitySave entity)
    {
        ...
        foreach(var variable in entity.CustomVariables)
        {
            ...
            if(shouldInlude)
            {
                var propertySave = CreatePropertySaveFrom(variable, entity);
    ...
    private TiledObjectTypePropertySave CreatePropertySaveFrom(CustomVariable variable, EntitySave entity)
    {
        ...
        var effectiveValue = variable.DefaultValue ?? entity.GetVariableValueRecursively(variable.Name);

        if(effectiveValue is float asFloat)
        {
            toReturn.DefaultAsString = GetTmxFriendlyValue(asFloat.ToString(CultureInfo.InvariantCulture), variable.Type);
        }
        else if (effectiveValue is double asDouble)
        {
            toReturn.DefaultAsString = GetTmxFriendlyValue(asDouble.ToString(CultureInfo.InvariantCulture), variable.Type);
        }
        else
        {
            toReturn.DefaultAsString = GetTmxFriendlyValue(effectiveValue?.ToString(), variable.Type);
        }
