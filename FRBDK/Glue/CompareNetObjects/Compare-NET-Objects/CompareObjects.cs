...
{
    public struct ComparisonDifference
    {
        public string FullObject1;
        public string FullObject2;
    ...
    public class CompareObjects
    {
        ...
        public ComparisonDifference GetComparisonDifference(string differenceString)
        {
            ComparisonDifference comparisonDifference = new ComparisonDifference();
            Regex firstArgRE = new Regex(@"([^ ]+)[ ]!=");
            comparisonDifference.FullObject1 = firstArgRE.Match(differenceString).Groups[1].Value;

            Regex secondArgRE = new Regex("!=[ ]([^ ]+)");
            comparisonDifference.FullObject2 = secondArgRE.Match(differenceString).Groups[1].Value;
            
            //Regex secondArgRE = new 
        ...
        private bool IgnoredByAttribute(IEnumerable attributes)
        {
            foreach (Attribute attribute in attributes)
            {
                if (AttributesToIgnore.Contains(attribute.GetType()))
                {
                    return true;
                }
            }
