...
### 2026-07-24 — NPC/Wizard test-coverage sweep (issue #1894 follow-up): build-smoke, WizardViewModel, combined scenarios, Gum standard-element audit

Follow-up to the "Make new-project creation (NPC) testable" and "Fix `RectangleCircleCodegenTests`" entries below. Five parallel workstreams closing the specific gaps those entries left open.

**NPC template build-smoke, beyond Desktop GL MonoGame:**
- **FNA and Web templates**: both build cleanly and now have real build-smoke coverage (`FnaProjectCreationSmokeTests.cs`, `WebProjectCreationSmokeTests.cs`, same pattern as `NewProjectCreationSmokeTests.cs`). The FNA template's "needs a net7 targeting pack" comment (written when the original NPC-testable PR added the Desktop-only test) was stale — it targets plain `net8.0-windows` with FNA's native deps as checked-in DLLs via `HintPath`, no exotic toolchain at all. The Web template (Blazor WASM + KNI `BlazorGL`) builds against the already-installed `wasm-tools`/`wasm-tools-net8` workloads.
- **Android**: real, confirmed dead end — not a missing-workload problem. The installed Android workload (36.1.69, SDK 10.0.302) has dropped `net8.0-android` support entirely (`microsoft.net.sdk.android`'s `WorkloadManifest.targets` unconditionally imports `Eol.targets` for `TargetFrameworkVersion <= 8.0`; only `Microsoft.Android.Ref.35`/`.36` reference packs are installed, no `.34`). Pinning the template to `net8.0-android34.0` was tried and fails identically — confirms it's not an API-level mismatch, the net8 Android surface is gone from this workload band. Fixing this for real means migrating the template and the `FlatRedBallAndroid`/MonoGame.Framework.Android package references to net9/net10-android — a real product migration, out of scope for a test-coverage pass. Root cause posted to tracking issue #1892.
- **iOS**: real, confirmed dead end, and not (only) the expected "needs a Mac" reason. Fails at restore before a build-host question is even reachable: under the default SDK band (10.0.302, workload manifest `ios 26.5.10284/10.0.100`) the TFM resolves to `net8.0-ios1.0` while the `FlatRedBalliOS` package requires `net8.0-ios17.2` (`NETSDK1202`/`NU1202`). Pinning to SDK 8.0.423 (workload manifest `ios 17.0.8478/8.0.100`, which does match the package's expectations) instead hits `NETSDK1147` — the workload resolver doesn't see `ios` as installed for the project despite `dotnet workload list` reporting it present. Not attempted further; likely needs a workload repair/reinstall for that SDK band, and would still need a paired macOS host afterward per the standard MAUI/iOS restriction.

**Wizard ViewModel and combined-scenario coverage:**
- `WizardViewModel.cs` (`OfficialPlugins/Wizard/ViewModels/WizardViewModel.cs`) has no validation/gating logic that blocks invalid option combinations — confirmed by direct investigation. It's pure `Get<T>`/`Set(value)` state with `[DependsOn]`-attributed computed booleans used only for WPF visibility binding. Constructing a bare `WizardViewModel` needs no Dispatcher/live window. Added `WizardViewModelTests.cs`: 27 theory-driven tests pinning the multi-condition computed properties (`ShowPlayerVsCloudCollision` — 4 inputs, mixed AND/OR; `ShowBorderCollisionCheckBox` — 4-way AND; `FollowPlayersWithCameraVisibility`, `IsAddGumScreenToLayerVisible`, `ShowPlayerVsSolidCollision`, `IsPlayerCreationSelectingOptions`) plus computed-on-computed chaining (`ShowAddPlatformAnimatorController`, which depends on another `[DependsOn]` property).
- Every prior Wizard test in this effort drove a single feature at a time. Added `WizardProjectLogicCombinedScenarioTests.cs`: one real `WizardProjectLogic.Apply(vm)` call with GameScreen + TiledMap + solid/cloud collision + HudLayer + a pre-seeded Gum project + player entity/list + collision relationships + camera controller all enabled together (the closest reachable approximation of `WizardViewModel.ApplyDefaults()`). Pins real cross-feature behavior an isolated test can't: collision-relationship `NamedObjectSave`s reference the `TileShapeCollection`s added earlier in the *same* `Apply()` call (proves ordering, not fixture data), every feature's `NamedObjectSave` coexists in `GameScreen` with no name collisions/overwrites, and `GameScreen.Generated.cs` contains real members from every included feature. Two `ApplyDefaults()` items dropped, both documented in the test file: `AddFlatRedBallForms` (already-known font-generator-process blocker) and real "Add Gum" plugin dispatch — a newly-found, deeper blocker: unlike Collision Plugin, `MainGumPlugin.StartUp` unconditionally calls `AddMenuItemTo` → `PluginBase.GetItem` → `GlueGui.MenuStrip.Items`, only ever populated by a live `MainGlueWindow`'s real menu tree. Worked around by pre-seeding the Gum project via `NewGumProjectCreationLogic` directly (same helper `GumProjectCreationTests` already uses) and leaving `vm.AddGum`'s own dispatch to harmlessly no-op inside the real `Apply()` call — same shape as the already-documented Tiled/Entity-Input-Movement plugin-dispatch gaps.

**Gum standard-element codegen audit, beyond Rectangle/Circle (issue #1907/#1908's bug class):**
Swept every remaining standard element for "backing FRB runtime type can't support a variable Gum's schema currently defines." Method: build the fixture via `StandardElementSave.Initialize(StandardElementsManager.Self.GetDefaultStateFor(name))` (never via project creation — stale embedded templates don't carry the current schema), feed to `StandardsCodeGenerator.Self.GenerateStandardElementSaveCodeFor`, inspect for members the backing type can't support.
- **Polygon** (`PolygonCodegenTests.cs`) — clean, not vulnerable. Never migrated onto Gum v3's fill/stroke/gradient family; `Red`/`Green`/`Blue`/`Alpha` already route through a dedicated custom getter/setter onto `LinePolygon.Color`, and `Points` is a `VariableListSave`, which neither codegen pipeline iterates at all (both only walk `.Variables`/`VariableSave`).
- **Text, ColoredRectangle** (`GumStandardElementCodegenSweepTests.cs`) — clean. ColoredRectangle never got the v3 fill/stroke/gradient family (only Rectangle/Circle did).
- **SolidRectangle** — not applicable. Not a registered standard element in `Gum.Managers.StandardElementsManager` at all (only `ColoredRectangle` exists there now; `GetDefaultStateFor("SolidRectangle")` throws). Pinned with a test asserting that absence rather than forcing the fixture pattern onto something that doesn't exist.
- **Arc, ColoredCircle, LottieAnimation, RoundedRectangle, Svg, Canvas** (`GumSkiaRenderableCodegenSweepTests.cs`) — all clean. Unlike Rectangle/Circle's outline-only `RenderingLibrary.Math.Geometry` backing, these six share `SkiaGum.Renderables.RenderableSkiaObject`, purpose-built with the full fill/stroke/gradient/dropshadow surface — every schema variable has a real backing member. One subtlety pinned along the way: the universal "Color" convenience property is XNA-`Color`-typed but these types back it with `System.Drawing.Color`; correctness depends on a version-gated (`FileVersion >= GluxVersions.GumUsesSystemTypes`) `ToXNA`/`ToSystemDrawing` conversion that's silent at the generated-source level (only real compilation catches a regression there) — worth knowing if this area is touched again. Needed adding `InternalsVisibleTo("GlueUnitTests")` to `GumPlugin.csproj` since `SkiaStandardElementsManager.AddSkiaStandards()` is internal and not part of `GlueTestBootstrap`'s always-on init.
- **NineSlice, Sprite, Container** — three real, live bugs found by this sweep and fixed separately (see the next entry): NineSlice's `Animate` property is version-gated out of the property pipeline but has no matching state-pipeline gate (CS0103 below the gate); Sprite's `RenderTargetTextureSource` was never skipped and generates against the wrong type (CS0029/CS0102 depending on version); Container's `SourceShaderFile` was never skipped and has no backing member at all (CS1061).
...
Last open item from #1894 (per PR #1902's writeup): `CreateGumProjectInternal(shouldAlsoAddForms: true, ...)`
- the `CreateGumProjectWithForms` call site - was undone because it unconditionally calls
`MainGumPlugin.HandleBuildMissingFonts`, which spawns an external process at the hardcoded relative path
`"Plugins/GumPlugin/Tools/GumProjectFontGenerator/GumProjectFontGenerator.exe"`, only ever populated there
by `Glue.exe`'s own build output.

**Turned out solvable for real, not just documentable.** `GumProjectFontGenerator.exe` is a real,
already-checked-in, headless console tool (its own "About" doc: built specifically to avoid an XNA/window
dependency, "will run on all platforms"; manually verified it prints an error and exits cleanly on bad args,
and exits 0 with no output against a font-less `.gumx`, no window ever created). `GlueUnitTests.csproj` now
carries a `<None Include>` item that mirrors `Glue.exe`'s own `Plugins/<PluginName>/Tools/...` deployment
layout under the test project's own output directory (alongside its existing `GumPlugin.csproj`
`ProjectReference`), so the real `Process.Start` call - not a fake - finds and runs the real exe during the
test. CI runs on `windows-latest`, so the standard Windows fonts (Arial, etc.) the generator needs are
present.

Reaching the font-generator call also needed the seam PR #1902 found and *reverted* (nothing to assert
against yet, at the time): adding a Forms component (`GumPluginCommands.AddComponent`) requires
`Gum.Managers.StandardElementsManager.Self.Initialize()`, else `GetDefaultStateFor` throws "You must first
call Initialize". Re-added as a new opt-in `GlueTestBootstrap.EnsureGumPluginStandardElementsInitialized()`
(same shape as `EnsureCollisionPluginAssetTypesRegistered`), which also calls
`GumPlugin.Managers.AssetTypeInfoManager.Self.AddCommonAtis()` per that PR's note - both are the same two
calls `MainGumPlugin.StartUp` makes, run directly instead of through a live plugin `StartUp`.

`GumProjectCreationTests.CreateGumProjectInternal_ShouldAddFormsComponentsAndGenerateFonts_WhenFormsRequested`
asserts five real side effects: the RFS's `IncludeFormsInComponents`/`IncludeComponentToFormsAssociation`
flags, a real `Button.gucx` written to disk under `Components/`, that component registered into the loaded
`GumProjectSave`, a real `.behx` behavior file written under `Behaviors/`, and - the part this PR unblocks -
real `.fnt` bitmap-font files generated under `FontCache/` (the embedded project template ships with no
`FontCache` files at all, so any `.fnt` found there was generated by the real external tool, not copied from
a template).

No production code changes. 155/155 fast tests (run twice) + 1/1 build-smoke (run twice) green.

**This closes #1894's full scope.** Every `PluginManager.CallPluginMethod`/`CallPluginMethodAsync` call site
in `WizardProjectLogic.cs` now has real test coverage, including the one variant (Gum "with forms") every
prior PR in this effort left as a documented, undone gap.
