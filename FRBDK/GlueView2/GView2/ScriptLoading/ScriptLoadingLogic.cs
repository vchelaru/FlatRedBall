...
namespace FlatRedBallWpf.ScriptLoading
{
    public class ScriptLoadingLogic
    {
        ...
        private void AddScreensAndEntities(string projectDirectoryFullPath, List<string> filesToCompile)
        {
            ...
            string[] allScreenFiles = new string[0];
            if (System.IO.Directory.Exists(screenDirectory))
            {
                allScreenFiles = System.IO.Directory.GetFiles(screenDirectory)
            ...
            string[] allEntityFiles = new string[0];
            if (System.IO.Directory.Exists(entitiesDirectory))
            {
                allEntityFiles = System.IO.Directory.GetFiles(entitiesDirectory)
                    .Where(item => item.Contains(".Generated."))
                    .ToArray();
