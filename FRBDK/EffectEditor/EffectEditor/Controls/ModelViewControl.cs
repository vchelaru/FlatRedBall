...
namespace EffectEditor.Controls
{
    public partial class ModelViewControl : FRBPanel
    {
        ...
        public Effect ApplyEffect(byte[] effectCode)
        {
            // Compile the effect
            Effect newEffect = new Effect(
                FlatRedBallServices.GraphicsDevice,
                effectCode, CompilerOptions.None, null);

            EffectEditor.EffectComponents.HLSLInformation.HlslSemantic sem =
                EffectEditor.EffectComponents.HLSLInformation.Semantics.VertexShader.Input[0];

            CurrentModel.CustomEffect = new EffectContainer(newEffect);
